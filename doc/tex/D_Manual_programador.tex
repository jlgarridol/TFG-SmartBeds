...
La carpeta \texttt{smartebs} están las fuentes de la aplicación web siendo el modulo raíz. Dentro de esta están los módulo \texttt{api} con la \texttt{API} web, \texttt{process} con los algoritmos de procesamiento de los datos, \texttt{resources} con la configuración y los \texttt{assets} de la página web (plantillas, códigos \texttt{JavaScript} etc), \texttt{routes} que engloba las rutas \textit{URI} para dirigir las peticiones. Por último, está la carpeta \texttt{test} con las pruebas unitarias.
...
Otras pruebas son las unitarias sobre la API, está se ejecuta con el fichero \texttt{/tests.py} y las pruebas de integración sobre la interfaz que están almacenadas en el fichero \texttt{/smartbeds/test/test.side} que se abren con el IDE de Selenium y se ejecutan en una versión local del servidor.xº